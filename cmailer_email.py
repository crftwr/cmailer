...
# listerから参照されなくなった時に ZipFile.close()を呼び出すためのラッパオブジェクト
...
## Zipファイルの仮想ディレクトリ中のファイルを表すアイテム
#
#  Zipファイルの仮想ディレクトリ中のファイルやディレクトリを意味するクラスです。\n\n
#
#  内骨格では、ファイルリストに表示されるアイテムを item_Xxxx という名前のクラスのオブジェクトで表現します。\n
#  
#  @sa item_Default
...
#  @sa item_Archive
#
class item_Zip(item_CommonPaint):

    def __init__( self, arc_filename, arc_obj, arc_dir, name, node ):
    ...
    def __unicode__(self):
        return u"%s:%s" % ( self.arc_filename, os.path.join( self.arc_dir, self.name ) )
    ...
    def open(self):
        ...
        path = os.path.join(self.arc_dir,self.name).replace("\\","/").encode("mbcs")
        fd = StringIO.StringIO( self.arc_obj.zipfile.read(path) )
        return fd

# Zipファイルの中のファイルをリストアップする機能
class lister_Zip(lister_Base):

    def __init__( self, main_window, arc_filename, arc_obj=None, arc_dir=u"", arc_tree=None ):

        if type(arc_filename)==type(''):
            arc_filename = unicode(arc_filename,'mbcs')
        if type(arc_dir)==type(''):
            arc_dir = unicode(arc_dir,'mbcs')
    ...
    def destroy(self):
        if self.arc_obj:
            self.arc_obj.release()

    def __call__( self ):

        # まだアーカイブをオープンしていなかったらオープンする
        if self.arc_obj==None or self.arc_tree==None:
            arc_tree = ArchiveNode( ( u"", 0, (0,0,0,0,0,0), ckit.FILE_ATTRIBUTE_DIRECTORY ) )
            arc_obj = ZipFileWrapper( zipfile.ZipFile( self.arc_filename, "r" ) )
            try:
                for name in arc_obj.zipfile.namelist():
                    if self.cancel_requested:
                        raise cfiler_error.CanceledError
                    info = arc_obj.zipfile.getinfo(name)
                    if type(name)==type(''):
                    ...
                    for i in xrange( len(names)-1 ):
                        ...
                        if not current_node.children.has_key(names[i]):
                            child = ArchiveNode( ( names[i], 0, info.date_time, ckit.FILE_ATTRIBUTE_DIRECTORY ) )
                            current_node.children[ names[i] ] = child
                        current_node = current_node.children[ names[i] ]
                    if names[-1]:
                        #print "name[%d]=%s" % (-1,names[-1])
                        child = ArchiveNode( ( names[-1], info.file_size, info.date_time, 0 ) )
        ...
        for node in current_node.children.itervalues():
    ...
    def __unicode__(self):
        return u"%s:%s" % ( self.arc_filename, self.arc_dir )
    ...
    def getCopy( self, name ):
        return ( lister_Zip( self.main_window, self.arc_filename, self.arc_obj, self.arc_dir, self.arc_tree ), name )

    def getChild( self, name ):
        return lister_Zip( self.main_window, self.arc_filename, self.arc_obj, ckit.normPath(os.path.join(self.arc_dir,name)), self.arc_tree )

    def getParent(self):
        if self.arc_dir:
            parent, name = os.path.split(self.arc_dir)
            return ( lister_Zip( self.main_window, self.arc_filename, self.arc_obj, parent, self.arc_tree ), name )
        else:
            parent, name = os.path.split(self.arc_filename)
            return ( lister_Default(self.main_window,parent), name )

    def getRoot(self):
        return lister_Zip( self.main_window, self.arc_filename, self.arc_obj, "", self.arc_tree )

    def locked(self):
        return False


# listerから参照されなくなった時に TarFile.close()を呼び出すためのラッパオブジェクト
...
## tar/tgz/bz2ファイルの仮想ディレクトリ中のファイルを表すアイテム
#
#  tar/tgz/bz2ファイルの仮想ディレクトリ中のファイルやディレクトリを意味するクラスです。\n\n
#
#  内骨格では、ファイルリストに表示されるアイテムを item_Xxxx という名前のクラスのオブジェクトで表現します。\n
#  
#  @sa item_Default
...
#  @sa item_Archive
#
class item_Tar(item_CommonPaint):
    ...
    def open(self):
        path = os.path.join( self.arc_dir, self.name ).replace("\\","/").encode("mbcs")
        try:
            fd = self.arc_obj.tarfile.extractfile(path)
        except:
            raise IOError
        return fd

# Tarファイルの中のファイルをリストアップする機能
class lister_Tar(lister_Base):
    ...
    def __call__( self ):
        ...
        if self.arc_obj==None or self.arc_tree==None:
            arc_tree = ArchiveNode( ( u"", 0, (0,0,0,0,0,0), ckit.FILE_ATTRIBUTE_DIRECTORY ) )
            arc_obj = TarFileWrapper( tarfile.open( self.arc_filename ) )
            try:
                for info in arc_obj.tarfile:
                    if self.cancel_requested:
                        raise cfiler_error.CanceledError
                    name = info.name
                    if type(name)==type(''):
                    ...
                    for i in xrange( len(names)-1 ):
                        #print "name[%d]=%s" % (i,names[i])
                        if not current_node.children.has_key(names[i]):
                            child = ArchiveNode( ( names[i], 0, time.localtime(info.mtime)[:6], ckit.FILE_ATTRIBUTE_DIRECTORY ) )
                            current_node.children[ names[i] ] = child
                        current_node = current_node.children[ names[i] ]
                    if names[-1]:
                        #print "name[%d]=%s" % (-1,names[-1])
                        if info.isdir():
                            ...
                            attr = 0
                        child = ArchiveNode( ( names[-1], info.size, time.localtime(info.mtime)[:6], attr ) )
                        current_node.children[ names[-1] ] = child
    ...
    def getCopy( self, name ):
        return ( lister_Tar( self.main_window, self.arc_filename, self.arc_obj, self.arc_dir, self.arc_tree ), name )

    def getChild( self, name ):
        return lister_Tar( self.main_window, self.arc_filename, self.arc_obj, ckit.normPath(os.path.join(self.arc_dir,name)), self.arc_tree )

    def getParent(self):
        if self.arc_dir:
            parent, name = os.path.split(self.arc_dir)
            return ( lister_Tar( self.main_window, self.arc_filename, self.arc_obj, parent, self.arc_tree ), name )
        else:
            parent, name = os.path.split(self.arc_filename)
            return ( lister_Default(self.main_window,parent), name )

    def getRoot(self):
        return lister_Tar( self.main_window, self.arc_filename, self.arc_obj, "", self.arc_tree )
...
class item_Archive(item_CommonPaint):
    ...
    def __init__( self, arc_filename, arc_dir, name, node ):
    ...
    def open(self):
        ...
        if self.node.info[1]>0:
            archiver = cfiler_archiver.getArchiver( os.path.splitext(self.arc_filename)[1] )
            archiver.extract( None, self.arc_filename, tmp_dir_path, os.path.join(self.arc_dir,self.name) )
        else:
            fd = file( tmp_file_path, "wb" )
            del fd

        fd = StringIO.StringIO( file(tmp_file_path,"rb").read() )
...
class lister_Archive(lister_Base):
    ...
    def __init__( self, main_window, arc_filename, arc_dir, arc_tree=None ):
    ...
    def __call__( self ):
        ...
        if self.arc_tree==None:
            arc_tree = ArchiveNode( ( u"", 0, (0,0,0,0,0,0), ckit.FILE_ATTRIBUTE_DIRECTORY ) )
            archiver = cfiler_archiver.getArchiver( os.path.splitext(self.arc_filename)[1] )
            arc_obj = archiver.openArchive( self.main_window.getHWND(), self.arc_filename, 0 )
            try:
    ...
    def getCopy( self, name ):
        return ( lister_Archive( self.main_window, self.arc_filename, self.arc_dir, self.arc_tree ), name )

    def getChild( self, name ):
        return lister_Archive( self.main_window, self.arc_filename, ckit.normPath(os.path.join(self.arc_dir,name)), self.arc_tree )

    def getParent(self):
        if self.arc_dir:
            parent, name = os.path.split(self.arc_dir)
            return ( lister_Archive( self.main_window, self.arc_filename, parent, self.arc_tree ), name )
        else:
            parent, name = os.path.split(self.arc_filename)
            return ( lister_Default(self.main_window,parent), name )

    def getRoot(self):
        return lister_Archive( self.main_window, self.arc_filename, "", self.arc_tree )
