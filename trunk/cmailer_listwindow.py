...
class ListWindow( ckit.Window ):
    ...
    def configure(self):
        ...
        self.keymap[ "C-Up" ] = self.command_CursorUpMark
        self.keymap[ "C-Down" ] = self.command_CursorDownMark
    ...
    def onChar( self, ch, mod ):
        ...
        if self.onekey_search:
            ...
            if 0x20<=ch<0x100:
                ...
                for i in xrange( self.select+1, len(self.items) ):
            ...
            if self.isearch:
                ...
                if ch==ord('\b'):
                    ...
                    isearch_newvalue = self.isearch.isearch_value + unichr(ch)
    ...
    def paint(self):
        ...
        for i in xrange(height):
            ...
            if index < len(self.items):
                ...
                self.putString( x, y+i, width, 1, attribute_normal, u" " * width )

        if self.status_bar:
            if self.isearch:
                attr = ckit.Attribute( fg=ckit.getColor("fg"))
                s = u" Search : %s_" % ( self.isearch.isearch_value )
    ...
    def command_CursorDownMark(self):
        select = self.select
        while True:
            select += 1
