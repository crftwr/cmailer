    ...
    def onCandidate( self, update_info ):
        ...
        if len(arg)>0:
            ...
            try:
                ...
                if unc[0]:
                    cmailer_misc.checkNetConnection(path)
...
class commandline_Calculator:
    ...
    def onEnter( self, commandline, text, mod ):
        ...
        if isinstance(result,int) or isinstance(result,long):
            result_string = u"%d" % result
        elif isinstance(result,float):
            result_string = u"%f" % result
        else:
            return False
    ...
    def onEnter( self, commandline, text, mod ):
        ...
        def base16to10(src):
            ...
            if i<0 or i>0xffffffff:
                raise ValueError
            if i>=0x80000000:
                dst = u"%d" % ( -0x80000000 + ( i - 0x80000000 ) )
            else:
                dst = u"%d" % i
            return dst    
        ...
        def base10to16(src):
            ...
            if i<-0x80000000 or i>=0x80000000:
                raise ValueError
            if i<0:
                dst = u"0x%08x" % ( 0x80000000 + ( i + 0x80000000 ) )
            else:
                dst = u"0x%08x" % i
            return dst
